...
const TgHeader = () => {
    ...
    const {branch_name} = useAppSelector(branchSelector)
    return (
        <header className="py-4 px-3 flex items-center w-full bg-tgHeader sticky top-0 z-10">
            ...
            <Flex align="center" justify="space-between" flex={1}>
                <div className="flex items-center">
                    {!state?.title && <Image src={'/icons/marker.svg'}/>}
                ...
                {state?.title &&
                    <div
                        className="h-full w-full text-xl text-white max-w-[60vw] text-end text-nowrap overflow-ellipsis truncate">{state.title}</div>}
            </Flex>
        </header>
    );
};
